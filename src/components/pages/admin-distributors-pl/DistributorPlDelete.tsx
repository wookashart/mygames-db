...
interface DistributorPlDeleteProps {
...
const DistributorPlDelete = ({
  editItem,
  open,
  handleClose,
  handleReloadData,
}: DistributorPlDeleteProps) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: () => {
      ...
      const input = {
        id: editItem?.distributor_pl_id,
      };

      fetch(`/api/distributor-pl-delete`, {
        headers: {
      ...
      <form onSubmit={formik.handleSubmit}>
        <Box mt={2}>
          <Typography color={customColors.textLight}>
            Czy na pewno chcesz usunąć polskiego wydawcę:{' '}
            <strong>{editItem?.distributor_pl_name}</strong>?
          </Typography>
        </Box>
...
export default DistributorPlDelete;
