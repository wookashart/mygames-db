...
interface DistributorCreateEditProps {
...
const DistributorCreateEdit = ({
  editItem,
  open,
  handleClose,
  handleReloadData,
}: DistributorCreateEditProps) => {
  ...
  const formik = useFormik({
    initialValues: {
      ...
      description: editItem ? editItem.distributor_description : '',
    },
    ...
    onSubmit: (values, { resetForm }) => {
      ...
      const input = {
        id: editItem ? editItem.distributor_id : null,
        name: values.name,
        description: values.description,
      };

      fetch(`/api/${editItem ? 'distributor-edit' : 'distributor-create'}`, {
        headers: {
  ...
  return (
    <Modal
      title={editItem ? 'Edytuj wydawcę' : 'Dodaj wydawcę'}
      open={open}
      size="md"
      handleClose={() => {
        setFormError({
...
export default DistributorCreateEdit;
