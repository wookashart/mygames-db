...
const WindowsTable = ({ items, handleOpenEditModal, handleOpenDeleteModal }: WindowsTableProps) => {
  return (
    <TableContainer>
      <Table sx={{ minWidth: 650 }} size="small" aria-label="dense-table">
        ...
        <TableBody>
          {items.map((row: WindowsData, index: number) => (
            <TableRow
              key={index}
              sx={{
            ...
            >
              <CellDefault value={row.windows_name} />
              <CellMenu>
                <Box>
                  <MenuItem
                    sx={{
