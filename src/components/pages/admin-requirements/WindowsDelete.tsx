...
const WindowsDelete = ({ editItem, open, handleClose, handleReloadData }: WindowsDeleteProps) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: () => {
      ...
      const input = {
        id: editItem?.windows_id,
      };

      fetch(`/api/windows-delete`, {
        headers: {
      ...
      <form onSubmit={formik.handleSubmit}>
        <Box mt={2}>
          <Typography color={customColors.textLight}>
            Czy na pewno chcesz usunąć windows: <strong>{editItem?.windows_name}</strong>?
          </Typography>
        </Box>
