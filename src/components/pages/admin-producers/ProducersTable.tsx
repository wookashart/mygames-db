  ...
  return (
    <TableContainer>
      <Table sx={{ minWidth: 650 }} size="small" aria-label="dense-table">
        ...
        <TableBody>
          {items.map((row: ProducerData, index: number) => (
            <TableRow
              key={index}
              sx={{
            ...
            >
              <CellLink label={row.producer_name} href={`/games?producer=${row.producer_id}`} />
              <CellDescription value={row.producer_description} />
              <CellMenu>
                <Box>
                  <MenuItem
                    sx={{
